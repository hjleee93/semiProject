...
<div class="container">
        ...
        <table class="tbl-reg">
            ...
            <tbody>
                <tr class="first">
                    ...
                    <td><input type="text" id="title" name="title" value="<%=r.getReviewTitle()%>"></td>
                </tr>
                <tr>
                    <th scope="row"><label for="writer">작성자</label></th>
                    <td><input type="text" name="writer" id="writer" value="<%=r.getReviewWriter() %>"></td>
                    <th scope="row"><label for="content">내용</label></th>
                    <td><input type="text" id="content" name="content" value="<%=r.getReviewContent() %>"></td>
                </tr>
                <tr>
                	<th></th>
                	<td></td>
                	<th scope="row"><label for="star">만족도</label></th>
                	<td colspan="2"><input type="text" name="mark" id="mark" value="<%=r.getReviewMark() %>"></td>
                </tr>
            </tbody>
        </table>
		
        <div class="board-view-btn">
        <input type="hidden" name="no" value="<%=r.getReviewNum() %>">
        	<a class="gray" id="modi"  onclick="Fn_modi();">수정</a>
            <a class="gray" id="cancel"  onclick="cancel_event();">취소</a>
        </div>
        </form>
    </div>
    ...
    <script>
        function cancel_event() {
                location.replace("<%=request.getContextPath()%>/reviewView");
        };
    	 function Fn_modi(){
    		 ...
    		 const frm= $("#reviewFrm");
			 const url="<%=request.getContextPath()%>/admin/reviewModiok";
			 frm.attr({
				 "action":url,
				 "method":"post",
			 });
